...
//	$(document).ready(function()
//	{
// 		$('#marry_yn').on('change',function()
//		{
// 			if( $('#marry_yn').val() == '0' )   //미혼일때
//          { 
// 				$('#child_num').val('0');
// 			}
// 		 });
...
/*
//db값 가져와서 처음에 뿌려줄때...... 흠....
        if( $('#area_fst').val() == '0' )  //임시 서울
        {
        	$('#area_sec_1').css("display",""); //서울지역2를 보이게하겠다
        } else if( $('#area_fst').val() == '1' ) //임시 경기
        { 
        	$('#area_sec_2').css("display",""); //경기지역2를 보이게하겠다
        }



//화면한번 뿌려준 상태에서 사용자가 area_fst변경하면 area_sec 변경되도록 이벤트 처리..
        $('#area_fst').on('change',function()
        {
        	$('[name=area_sec]').css("display","none"); //일단 전체 감추고
        	
        	if( $('#area_fst').val() == '0' ) 
        	{ //임시 서울
            	$('#area_sec_1').css("display",""); //서울지역2를 보이게
            } else if( $('#area_fst').val() == '1' ) 
            { //임시 경기
            	$('#area_sec_2').css("display",""); //경기지역2를 보이게
            }
        });
        // --> display 보이는 애들만 값(1차에서 선택된 애들 것만 2차에 보일 수 있도록! 
        	  select에 해당하는 option-value 가져가서 업데이트하기
								    	...
								    	<tbody>
								    		...
								    		<tr>
								    			...
								    			<td class="mobile" style="width: 100px; text-align: center;">
								    				...
								    				<%-- <input type="text" name="tel" class="text-field form-control" value="${user_tel }" disabled="disabled"> --%>
								    				<!-- 본인인증 못해서 업데이트 잘되나 확인하려고 disabled 없애고 직접 번호 넣어서 테스트해봄.. 다시 없앰 .. -->
								    			</td>
								    			<td class="mobile" style="text-align: center;">
								    				<button type="button" class="btn btn-success" onclick="newPage()"> 본인인증 </button>
								    			</td>
